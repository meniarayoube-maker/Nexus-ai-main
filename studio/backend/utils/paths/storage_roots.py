...
def resolve_training_write_dir(path_value: str | None = None) -> Path:
    """Resolve a training output / checkpoint directory.

    Absolute paths are accepted (e.g. Google Drive on Colab, external disk).
    Relative paths stay under outputs_root().
    """
    if not path_value or not str(path_value).strip():
        return outputs_root()
    raw = str(path_value).strip()
